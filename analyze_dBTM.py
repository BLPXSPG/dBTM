...
if __name__ == "__main__":
    ...
    for time in time_choice:
        ...
        score_indices, brand_map_indices, score_initial = load_score_data(source_dir, time)
        brand_score_real = get_brand_score_real(score_indices, brand_map_indices, brand_map)

        # Load BTM parameters.
        if time == max(time_choice):
            param_dir = os.path.join(source_dir, "btm-fits", str(time), "params")
        else:
            param_dir = os.path.join(source_dir, "btm-fits", str(time), "test")
