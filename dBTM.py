...
def build_input_pipeline(source_dir,
    ...
    num_documents, num_words, counts, brand_indices, score_indices, num_brands = load_data(
        train_dir)
    iterator, dev_iterator = build_database(
        random_state, num_documents, counts_transformation, counts, brand_indices, score_indices, batch_size, dev_ratio)
    if test:
    ...
    vocabulary = np.loadtxt(os.path.join(data_dir, "vocabulary.txt"),
                            ...
                            delimiter="\n",
                            comments="<!-")
    brand_score_real = get_brand_score_real(
        score_indices, brand_indices, brand_map)
...
def check_inf_nan(input_tensor):
    # input_tensor[input_tensor.isnan()] = 0.
    input_tensor[np.isnan(input_tensor)] = 0.
    #input_tensor[input_tensor.isinf()] = 1.
...
def get_distribution(source_dir, time_slice, num_documents, num_words, random_state, rollback=0.1):
    ...
    fit_dir = os.path.join(source_dir, "btm-fits",
    ...
    objective_topic_loc = rollback*initial_objective_topic_loc + \
        (1-rollback)*objective_topic_loc
...
def get_rate(correlation, set_size):
    mu = np.log((1+correlation)/(1-correlation))
    mu = mu**0.5
    sigma = 1/(set_size-3)
    correlation_sampled = np.random.normal(mu, sigma)
...
def main(argv):
    ...
    for time_slice in time_choice:
        ...
        document_indices, counts, brand_indices, score_indices = tf.cond(
            dev_check, lambda: dev_iterator.get_next(), lambda: iterator.get_next())
                ...
                if step % elbo_check_steps == 0 or step == current_max_steps - 1 or step == current_min_steps:
                    ...
                    (document_loc_val, document_scale_val, objective_topic_loc_val,
                     ...
                     ideal_point_scale_val, elbo_val, score_loss_val, ) = sess.run([
                         ...
                         ideological_topic_scale, ideal_point_loc, ideal_point_scale, elbo, score_loss, ], feed_dict={test_check: False, dev_check: True})

                    # Now check the spearman
                    # Print ideal point orderings.
                    brand_score_generated = {}
                    for index in range(len(brand_map)):
                        brand_score_generated[brand_map[index]
                                               ] = ideal_point_loc_val[index]
                    rating_real = []
                    rating_generated = []
                    for brand_i in brand_score_real:
                        rating_real.append(brand_score_real[brand_i])
                        rating_generated.append(
                            brand_score_generated[brand_i])
                    ranking_correlation, ranking_p_value_real = spearmanr(
                        rating_real, rating_generated)
                    z_mean = (tf.math.log((1+abs(ranking_correlation)
                                           )/(1-abs(ranking_correlation))))*0.5
                    ...
                    ranking_p_value_current = z_dist.cdf(
                        abs(ranking_correlation_previous))
                    (ranking_p_value_current_val,) = sess.run(
                        [ranking_p_value_current, ], feed_dict={test_check: False, dev_check: True})
                    ranking_p_value_current = max(
                        0.05, ranking_p_value_current_val)
                    print("cor", ranking_correlation, ranking_p_value_real,
                          ranking_p_value_current, ranking_p_value_current_val)
                    ...
                    if ranking_p_value_current < ranking_p_value:
                        ...
                        if step < current_min_steps:
                            ...
                            if time_slice in train_steps_recording:
                                train_steps_recording[time_slice] += [step, "Training stage, Ranking Result: {:.3f} (p-value: {:.3f})".format(
                                    ranking_correlation, ranking_p_value_real)]
                            else:
                                train_steps_recording[time_slice] = [
                                    step, "Training stage, Ranking Result: {:.3f} (p-value: {:.3f})".format(ranking_correlation, ranking_p_value_real)]
                            break
                    ...
                    if ranking_p_value_current < ranking_p_value:
                        ...
                        if step < min_add_steps:
                            ...
                            print("Early Stop at Add Step: {:>3d} time_slice: {:.1f}, Ranking Result: {:.3f} (p-value: {:.3f})".format(
                                step, time_slice, ranking_correlation, ranking_p_value_current))
                            if time_slice in train_steps_recording:
                                train_steps_recording[time_slice] += [step, "Add stage, Ranking Result: {:.3f} (p-value: {:.3f})".format(
                                    ranking_correlation, ranking_p_value_real)]
                            else:
                                train_steps_recording[time_slice] = [
                                    step, "Add stage, Ranking Result: {:.3f} (p-value: {:.3f})".format(ranking_correlation, ranking_p_value_real)]
                            break
