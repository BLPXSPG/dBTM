...
def build_input_pipeline(source_dir,
    ...
    counts = sparse.load_npz(os.path.join(train_dir, "counts.npz"))
    num_documents, num_words = counts.shape
    author_indices = np.load(
        os.path.join(train_dir, "author_indices.npy")).astype(np.int32)
...
def main(argv):
    ...
    data_dir = os.path.join(source_dir, "clean")
    save_all_dir = os.path.join(source_dir, "tbip-fits")
    if tf.gfile.Exists(save_all_dir):
        tf.logging.warn("Deleting old log directory at {}".format(save_all_dir))
        tf.gfile.DeleteRecursively(save_all_dir)
    tf.gfile.MakeDirs(save_all_dir)
    ...
    for time_slice in time_choice:
        ...
        save_dir = os.path.join(save_all_dir, str(time_slice))
    ...
    """
        ...
        with tf.Session() as sess:
            ...
            for step in range(FLAGS.max_steps):
                ...
                if step % 10000 == 0:
                    param_save_dir = os.path.join(save_dir, "middle", str(step))
                    if not tf.gfile.Exists(param_save_dir):
                        tf.gfile.MakeDirs(param_save_dir)
