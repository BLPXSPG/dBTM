...
def save_topics(objective_topic_loc,
                 objective_topic_scale,
                 ideological_topic_loc,
                 ideological_topic_scale,
                 vocabulary,
  ...
  for topic_idx in range(num_topics):
    ...
    topic_strings.append([negative_row, neutral_row, positive_row])

  return topic_strings


def topic_words(objective_topic_loc,
                 objective_topic_scale,
                 ideological_topic_loc,
                 ideological_topic_scale,
                 vocabulary,
  ...
  for topic_idx in range(num_topics):
    # print("neg, neutral, pos mean: ", topic_idx, negative_mean, neutral_mean, positive_mean)
    print("#################", topic_idx, "##################")
    # neutral_start_string = "Neutral {}:".format(topic_idx)

    for i in range(len(topic_means)):
      mean_i = topic_means[i]
      top_words = np.argsort(-mean_i, axis=1)
  ...
  '''with open(save_dir, "w") as f:
    for topic_idx in range(num_topics):
      ...
      for i in range(len(topic_means)):
        ...
        words_row = [vocabulary[word] for word in 
                      top_words[topic_idx, :words_per_topic]]
        if print_topic: print(i, words_row)
        word_string = " ".join(words_row)
        word_string = word_string.split()
        word_string = list(set(word_string))
        word_string = " ".join(word_string[:10])
        # print("save to file", save_dir)
        f.write(word_string)
        f.write('\n')
        word_string_all.append(words_row)
